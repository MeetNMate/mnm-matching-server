...
def main():
    ...
    try:
        insert_profiles(df, name_list)
        insert_matching_info(df)
    except Exception as e:
        print("[DB에 데이터 삽입 중 에러 발생] " + e)

def create_name_list():
    name_list = "남민영, 최은혜, 김서윤, 신신혜, 설종진, 류명수, 안영환, 서민욱, 백상수, 홍형민, 조도훈, 안우재, 권문식, 손용호, 안수현, 양한길, 강우람, 제갈한결, 허으뜸, 전힘찬, 전한길, 제갈달, 노믿음, 성미르, 류나길, 강믿음, 손나길, 황보달, 이나래, 정한별, 배빛나, 최달래, 안구슬, 정누리, 류보람, 조바다, 백아라, 전아람, 강별빛, 윤보다, 송새론, 남궁새벽, 조훈, 정호, 강혁, 제갈웅, 정혁, 정훈, 장호, 한경택, 전무열, 전이수, 설치원, 송승헌, 풍현승, 임철순, 문요한, 고광조, 사공덕수, 문성한, 서남규, 표남규, 심광조, 추경택, 고요한, 김덕수, 정애정, 유지해, 배장미, 사공지해, 허영신, 최재인, 권숙자, 안가영, 정미란, 황보지해, 심혜림, 송혜린, 성나영, 손경완, 남궁유리, 예혜린, 표은채, 전나영, 최이경, 제갈유리"
...
def preprocess_data(df):
    ...
    # mean으로 결측치 채움
    df['user_bug_killer'] = df['user_bug_killer'].fillna(int(df['user_bug_killer'].mean()))
    ...
    # mean으로 결측치 채움
    df['share_item'] = df['share_item'].fillna(int(df['share_item'].mean()))
    
    df['mate_alcohol'] = df['mate_alcohol'].apply(lambda x : pre_mate_alcohol(x))
    # mean으로 결측치 채움
    df['mate_alcohol'] = df['mate_alcohol'].fillna(int(df['mate_alcohol'].mean()))
    
    df['mate_clean'] = df['mate_clean'].apply(lambda x : pre_mate_clean(x))
    # mean으로 결측치 채움
    df['mate_clean'] = df['mate_clean'].fillna(int(df['mate_clean'].mean()))
    
    df['permission_to_enter'] = df['permission_to_enter'].apply(lambda x : pre_permission_to_enter(x))
    
    return df

def insert_profiles(df, name_list):
    conn = pymysql.connect(host = 'project-mnm-db.ctixol1fxh7r.ap-northeast-2.rds.amazonaws.com', 
                       port = 3306, 
                       user = 'root', 
    ...
    for i in range(72):
        data = (i+1, i+1, name_list[i], df.loc[i, 'sex'], df.loc[i, 'age'])
        query = "INSERT INTO profiles (id, uid, name, sex, age, description, image, score) VALUES (%s, %s, %s, %s, %s, '저와 함께 살 멋쟁이를 구합니다.', 'images/profile/default.png', 0);"
...
def insert_matching_info(df):
    ...
    query = "INSERT INTO matching_infos (uid, mbti, user_smoking, mate_smoking, user_pet, user_bug_killer, mate_bug_killer, user_cooking, mate_cooking, eat_together, share_item, mate_alcohol, mate_clean, permission_to_enter, user_pet_dog, user_pet_cat, user_pet_reptile_fish, user_pet_rodent, user_pet_bird, mate_pet, mate_pet_dog, mate_pet_cat, mate_pet_reptile_fish, mate_pet_rodent, mate_pet_bird, air_like_airconditioner, air_like_heater, noise_talking, noise_music, noise_alarm, id) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);"
